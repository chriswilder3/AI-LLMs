...
def create_chatbot():
    ...
    con = sqlite3.connect("checkpoint.db",check_same_thread= False)

    checkpointer1 = SqliteSaver(conn = con)
    chatbot_workflow = graph.compile(checkpointer= checkpointer1)
    return chatbot_workflow, checkpointer1

def main():
    # Keep the creation outside the loop otherwise, new workflow
    # created everytime and hence persistance fails
    workflow, checkpointer = create_chatbot()
